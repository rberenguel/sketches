  ...
  function plot() {
    ...
    for (let i = 0; i < SKYSTEPS; i++) {
      for (let j = 0; j < SKYSTEPS; j++) {
        ...
        scene.circle(x, y, scene.random(0.3 * W / SKYSTEPS, 2 * W / SKYSTEPS))
      }
    }
    // Ground
    scene.fill(5, 72, 16)
    ...
    for (let i = 0; i < GROUNDSTEPS; i++) {
      for (let j = 0; j < GROUNDSTEPS; j++) {
        ...
        const y = 0.6 * H + 0.4 * j * H / GROUNDSTEPS + scene.random(0.4 * H / GROUNDSTEPS)
        //console.log(x, y)
        const smoothing = smoothStep(0, SKYSTEPS, j)
        const f = Math.sqrt(Math.sqrt(smoothing))
        if (scene.random() > f + 0.3) {
        ...
        scene.circle(x, y, scene.random(0.3 * W / GROUNDSTEPS, 2 * W / GROUNDSTEPS))
      }
    }
    const DSTEPS = 5
    const MAXWSTEPS = 12
    const DGAP = 0.38 / DSTEPS
    for (let i = 0; i < DSTEPS; i++) { // Depth steps
      const WSTEPS = MAXWSTEPS * (1 - smoothStep(0, DSTEPS, i))
      const WGAP = 1.0 / WSTEPS
      let prevW = 0
      for (let j = 0; j < WSTEPS; j++) { // Width steps
        const tw = scene.random(0.03 + 0.015 * smoothStep(0, DSTEPS, i), 0.03 + 0.04 * smoothStep(0, DSTEPS, i))
        const wpos = prevW + scene.random(0, WGAP)
        const hpos = scene.random(i * DGAP, (i + 1) * DGAP)
        prevW = wpos + tw
        const c = (wpos + tw / 2)
        //scene.fill(55, 2, 31)
        const hue = 30 + scene.random(15 * smoothStep(0, 10, i), 25 * smoothStep(0, 10, i))
        const sat = 2 + scene.random(20 * smoothStep(0, 10, i), 30 * smoothStep(0, 10, i))
        const bri = 25 + scene.random(10 * smoothStep(0, 10, i), 15 * smoothStep(0, 10, i))
        scene.push()
