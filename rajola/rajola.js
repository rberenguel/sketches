...
import { getLargeCanvas, sceneShuffle, signature } from "../libraries/misc.js";
...
const sketch = (s) => {
  ...
  const lightHex = ["#c8d1d0", "#c7a27f", "#b3a291", "#c6b599", "#aaaaac"];
  function h2vGL(sc, h) {
  ...
  function plot() {
    ...
    for (let c of [
      shuffledPalette[0],
      shuffledLightPalette[0],
      shuffledPalette[1],
      shuffledLightPalette[1],
      shuffledPalette[2],
      shuffledLightPalette[2],
    ]) {
      const [r, g, b] = c;
  ...
  function createGUI() {
    ...
    cfg.info =
      "Inspired by <a href='https://www.theguardian.com/cities/2018/jun/19/the-tile-hunter-of-barcelona-preserving-a-unique-form-of-local-art-in-pictures'>Catalan hydraulic tiles</i> (<i>cement carpets</i>)";
    cfg.subinfo =
      "The palette is extracted from images of real tiles. All is drawn in a GLSL shader. The patterns are in essence <a href='https://en.wikipedia.org/wiki/Ernst_Chladni#Chladni_figures'>Chladni patterns</a> although the <code>k</code> parameter brings into play other, different patterns (similar formulas to Chladni's).<hr/><code>m</code> and <code>n</code> control periods, <code>p</code> and <code>q</code> amplitudes (they map to <code>a</code> and <code>b</code> in the usual Chladni formulation), <code>k</code> changes the formula smoothly.";
    cfg.s = s;
    R = new Key("r", () => {
      gui.spin(() => {
        s.clear();
    ...
    let aControl = new Control([decA, incA], "+/- a", aInt);
    let incB = new Key(
      "Q",
      () => {
        b += 0.1;
    ...
    let bControl = new Control([decB, incB], "+/- b", bInt);
    let incM = new Key(
      "M",
      () => {
        m += 0.1;
    ...
    let mControl = new Control([decM, incM], "+/- m", mInt);
    let incN = new Key(
      "N",
      () => {
        n += 0.1;
    ...
    let nControl = new Control([decN, incN], "+/- n", nInt);
    let incK = new Key(
      "K",
      () => {
        k = (k + 0.5 + 40) % 40;
    ...
    let kControl = new Control([decK, incK], "+/- k", kInt);
    let SP = new Key("i", () => {
      quat = !quat;
      R.action();
    });
    let spString = new String(() => (quat ? "Split" : "Not split"));
    let spControl = new Control([SP], "Split in 4 or not", spString);
