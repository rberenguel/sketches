...
import { getLargeCanvas, addText, smoothStep } from "../libraries/misc.js";

import { ctxGranulateChannels } from "../libraries/effects.js";

import { easeInSq } from "../libraries/eases.js";
...
const sketch = (s) => {
    ...
    for (let i = 1; i < length; i++) {
      for (let j = 0; j < 200; j++) {
        if (scene.random() < 0.5 / i) {
          ...
          scene.circle(
            x + i * Math.cos(angle) + R * Math.cos(th),
            y + i * Math.sin(angle) + R * Math.sin(th),
            1.5 * cfg.hd,
          );
        }
      }
  ...
  function scenery() {
    ...
    if (!maskIt) {
      addText(
        textCfg,
        scene.width - 10,
        textCfg.fontsize * cfg.hd + 3,
        identifier,
      );
      addText(
        textCfg,
        scene.width - 10,
        textCfg.fontsize * cfg.hd * 2 + 3 * cfg.hd,
        "rb'23",
      );
    } else {
      const textParams = {
  ...
  const createGUI = (gui) => {
    ...
    cfg.info =
      "Reusing some  code from 70s-Patch but improving some parts, particularly the mountains";
    cfg.subinfo =
      "This goes well with <a href='https://gavinharrisonsounds.bandcamp.com/track/groove-overload'>Groove overload</a> by Gavin Harrison<br/>Very high resolutions can fail depending on the browser";
    cfg.s = s;
    let R = new Key("r", () => {
      gui.spin(() => {
        cfg.s.clear();
