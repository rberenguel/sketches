...
const sketch = (s) => 
{
	...
	scene.stroke(col)
			...
			/*if(scene.random()< 0.01){
	const col = scene.color(hu, 20, scene.randomGaussian(40, 10), 0.2)
				//scene.stroke(col)
	scene.fill(col)
	scene.noStroke()
      }*/
	...
	for (let i = 0; i < W / cfg.hd; i++) {
			for (let j = 0; j < H / cfg.hd; j++) {
				let cc = backdrop.color((scene.randomGaussian(240, 100)+360) % 360, scene.randomGaussian(15, 10), scene.randomGaussian(80, 10))
				backdrop.fill(cc)
				backdrop.stroke(cc)
