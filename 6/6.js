...
import { getLargeCanvas, copyColor, signature } from "../libraries/misc.js";
...
const sketch = (s) => {
  ...
  function nave(scene, config) {
    ...
    scene.drawingContext.setLineDash([
      scene.random(4, 6) << 0,
      scene.random(13, 16) << 0,
    ]);
  ...
  function apse(scene, config) {
    ...
    for (let i = 0; i <= divs; i++) {
      // This is duplicating the columns above
    ...
    for (let i = 0; i < innerPillars.length - 1; i++) {
      ...
      if (i == 0)
        sketchedLine(scene, sx, sy, bx, y, scene.color("black"), rn());
    ...
    for (let i = 0; i < outerPillars.length - 1; i++) {
      ...
      if (radialCond(i)) {
        ...
        for (let j = 0; j < radi.length - 1; j++) {
          ...
          if (j != 0)
            support(scene, sx, sy, 0.5 * csize, 1.5 * wsize, PI / 2 + sa);
        }
        let [sx, sy, sa] = radi[0];
  ...
  function transept(scene, config) {
    //}, x, y, w, h, wsize){
    const [x, y] = config.transeptCenter;
  ...
  s.draw = () => {
      ...
      for (let i = 0; i < W; i += 1) {
        for (let j = 0; j < H; j += 1) {
          scene.fill(
            scene.color(
              60,
              30 * scene.noise((15 * i) / W, (20 * j) / H),
              50 + 20 * scene.noise((12 * i) / W, (18 * j) / H),
              0.5,
            ),
          );
          scene.circle(i, j, 3);
    ...
    cath.transeptCenter = [
      cath.naveCenter[0] + 0.5 * (cath.naveSize[0] + cath.transeptSize[0]),
      cath.naveCenter[1],
    ];
