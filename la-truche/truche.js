...
import { getLargeCanvas, copyColor } from "../libraries/misc.js";

import { olderHD, oldieHD } from "../libraries/effects.js";
...
const sketch = (s) => {
  ...
  function loc(scene, avoid) {
    ...
    const syllableStart = ["tru", "pe", "fra", "bou", "que"];
    const syllableEnd = ["che", "sce", "ise", "tins", "que"];
  ...
  function plot() {
    ...
    let x = scene.random(scene.width / 10, scene.width / 4),
      y = scene.random(scene.height / 10, scene.height / 4);
    ...
    (x = scene.random(scene.width / 2, (3 * scene.width) / 4)),
      (y = scene.random(scene.height / 3, (3 * scene.height) / 4));
  ...
  function wavyArc(scene, x, y, r, start, end, fill) {
    ...
    for (let i = 1; i < incs; i++) {
      let rr =
        r +
        (-fac +
          2 * fac * scene.noise(((i * x) / 20) << 0, ((i * y) / 20) << 0)); // Needs hd
  ...
  function createGUI() {
    ...
    let gui = new GUI(
      "La Truche, RB 2023/04 \u{1F1E8}\u{1F1ED}",
      info,
      subinfo,
      [saveCmd, resetCanvas],
      [hdControl],
