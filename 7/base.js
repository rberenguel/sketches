...
const sketch = (s) => {
  ...
  s.draw = () => {
    ...
    scene.vertex(s.bezierPoint(...upxs, 1), s.bezierPoint(...upys, 1), 0, 0, 0); //s.map(y, 0, H, 0, 1))
    for (let i = 99; i >= 0; i--) {
      const t = i / 100;
    ...
    scene.vertex(s.bezierPoint(...loxs, 1), s.bezierPoint(...loys, 1), 0, 0, 0); //s.map(y, 0, H, 0, 1))
    for (let i = 1; i <= 100; i++) {
      const t = i / 100;
