...
import { getLargeCanvas, smoothStep, signature } from "../libraries/misc.js";
...
const sketch = (s) => {
  ...
  const createGUI = (gui) => {
    ...
    cfg.info =
      "Lined stars inspired by a Christmas postcard I did in 2008 in PostScript. The snow effect reuses the stars from Synthwave";
    cfg.subinfo =
      "Press the key/tap/click on the letters to change the parameters, and press <code>x</code> and <code>r</code> to redraw.";
    cfg.s = s;
    R = new Key("r", () => {
      gui.spin(() => {
        cfg.s.clear();
    ...
    let showBuildingsBoolControl = new Control(
      [B],
      "buildings?",
      showBuildingsBool,
    );
    ...
    let wControl = new Control([decW, incW], "+/- star stroke", wInt);
    let incL = new Key("]", () => {
      layers += 5;
    });
    let decL = new Key("[", () => {
    ...
    let lControl = new Control([decL, incL], "+/- layers", lInt);

    cfg.commands = [resetCanvas, cfg.seeder.command];
    cfg.controls = [
      wControl,
      lControl,
      showBuildingsBoolControl,
      cfg.seeder.control,
    ];
