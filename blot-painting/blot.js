...
const sketch = (s) => {
  ...
  s.setup = () => {
    ...
    p5canvas.mousePressed(() => {
      ...
      if (touched >= 0) {
        ...
        if (randomColor) ink = palette[mod(touched - 1, palette.length)];
        background.clear();
        drawBlot(
          {
            s: s,
  ...
  function createGUI() {
    let info = `Tap/click on the canvas to trigger an ink blot. It will take a bit (it's expensive to compute)`;
    ...
    let rndColorControl = new Control(
      [R],
    ...
    let paintControl = new Control([T], "Paint/Field", paintString);
    ...
    let potentialControl = new Control(
      [F],
      "generate field strength",
      potentialBool,
    );
