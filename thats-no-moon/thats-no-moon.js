...
const sketch = (s) => {
    ...
    for (let i = 0; i < npts; i++) {
      ...
      if (scene.random() < C_DENSITY) {
        ...
        const col = scene.color(
          hu,
          scene.randomGaussian(20, 40),
          scene.randomGaussian(60, 20),
          0.1 + scene.randomGaussian(0.5, 0.2),
        );
        scene.stroke(col);
  ...
  const createGUI = (gui) => {
    ...
    cfg.subinfo =
      "Not fully resolution independent, but not too bad either if increased.";
    cfg.s = s;
    R = new Key("r", () => {
      gui.spin(() => {
        cfg.s.clear();
