...
const sketch = (s) => {
    ...
    class Particle {
        ...
        move(scene) {
            ...
            const nd = scene.random(0.3*this.dist, this.dist)
            let v = p5.Vector.fromAngle(theta, nd)
            this.x += v.x;
            this.y += v.y;
        }
        draw(scene) {
            ...
            const ns = scene.random(0.4 * this.size, 1.5 * this.size)
            scene.rect(this.x, this.y, 14*ns, 6*ns, 2*ns)
        }
        stop(scene) {
