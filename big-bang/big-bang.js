...
const sketch = (s) => {
  ...
  function lin(scene, x0, y0, x1, y1, npts, rec, flg) {
    ...
    for (let i = 0; i < npts; i++) {
      ...
      if (scene.random() < 0.01) {
        ...
        const col = scene.color(hu, sa, br, scene.random(0.4));
        scene.noFill();
        scene.stroke(col);
      }
      if (scene.random() < 0.1) {
      ...
      if (scene.random() < 0.01) {
        const col = scene.color(hu, 100, scene.randomGaussian(70, 10), 0.4);
        scene.stroke(col);
        scene.fill(col);
        scene.noStroke();
      }
      if (scene.random() < 0.001) {
        const col = scene.color(
          hu,
          scene.randomGaussian(30, 60),
          scene.randomGaussian(70, 20),
          scene.random(0.4),
        );
        scene.stroke(col);
  ...
  const createGUI = (gui) => {
    cfg.title = "Big Bang, RB 2023/8 \u{1F1E8}\u{1F1ED}";
