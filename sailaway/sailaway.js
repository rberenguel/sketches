...
import { c82GeoPrimesPalette } from "../libraries/palettes.js";
...
const sketch = (s) => {
  ...
  function circ(scene, [cx, cy], [cw, ch]) {
    ...
    for (let rep = 0; rep < repeats; rep++) {
      ...
      for (let i = 0; i < steps; i++) {
        ...
        const x =
          cx +
          scene.randomGaussian(k * 0.5 * cw, 0.15 * cw) *
            Math.cos((i * 2 * PI) / steps);
        const y =
          cy +
          scene.randomGaussian(k * 0.5 * ch, 0.15 * ch) *
            Math.sin((i * 2 * PI) / steps);
        //scene.circle(x, y, 3)
    ...
    for (let reps = 0; reps < repeats; reps++) {
      for (let i = 0; i < HSTEPS; i++) {
        for (let j = 0; j < WSTEPS; j++) {
          ...
          const x =
            bx + (j * (ux - bx)) / WSTEPS + scene.randomGaussian(0, 4 * factor);
          const y =
            by + (i * (uy - by)) / HSTEPS + scene.randomGaussian(0, 4 * factor);
  ...
  const createGUI = (gui) => {
    ...
    cfg.info =
      "Started adding some very translucent rectangles in circles, got clouds for some reason and then, this happened.";
