...
const sketch = (s) => {
    ...
    function star(gr, p) {
        ...
        let transpNight = s.color(s.red(night), s.green(night), s.blue(
            night), 0.9)
        ...
        for (let i = 0; i < p * 7.0; i++) {
            let colour = s.lerpColor(starColour, transpNight, i / (p *
                10.0))
            gr.stroke(colour)
    ...
    function moon(moonColour) {
        ...
        let transpMoonlightBorder = s.color(s.red(moonlightBorder), s.green(
            moonlightBorder), s.blue(
            moonlightBorder), 0.001)
        let [x, y] = moonPosition
        for (let i = 0; i < 30; i++) {
            let colour = s.lerpColor(moonColour, transpMoonlightBorder, i /
                30.0)
            s.fill(colour)
            s.circle(x, y, i * 3.2)
        }
    ...
    function moonlight(g, moonColour, alpha, factor1, factor2) {
        let transpNight = s.color(s.red(night), s.green(night), s.blue(
            night), 0)
        let transpMoonlightBorder = s.color(s.red(moonColour), s.green(
            moonColour), s.blue(
            moonColour), alpha)
        let [x, y] = moonPosition
        g.beginShape()
    ...
    function layer(k, dark) {
        ...
        for (let i = 0; i <= pts; i++) {
            s.curveVertex(
                i * s.width / pts,
                1 * s.height / 3 + 2 * s.noise((i + 1) * (3 +
                    k)) * (Math.sqrt(k) * s.height / 6))
    ...
    function generate() {
        ...
        moonPosition = [s.randomGaussian(800, 100), s.randomGaussian(200,
            50)]
        ...
        for (let k = 1; k <= 4; k++) {
            ...
            let dark = s.color(r + k, g + s.noise(k) * 15, b + s.noise(
                k) * 65)
    ...
    function createGUI() {
        ...
        gui.withTitle("Crazy, crazy nights, RB 2020/05")
            .withInfo(info)
            .withSubinfo(subinfo)
            .withCommands([saveCmd,
                resetCanvas
            ])
