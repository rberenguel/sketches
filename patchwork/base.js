...
const sketch = (s) => {
  ...
  function flower(scene, x, y, r, sew){
    ...
    const baseAngles = [0, -PI/4, -PI/2, -PI, -3*PI/2, -(2*PI-PI/4)]
  ...
  s.draw = () => {
    ...
    stalkMask.noStroke()
    flower(flowerMask, 0.5*scene.width, 0.3*scene.height, 0.15*scene.height)
    stalk(stalkMask, 0.5*scene.width, 0.4*scene.height, 0.02*scene.height)
    ...
    textile(greenCloth, 0.3*scene.width, 0, 0.5*scene.width, scene.height, 1, 8, ["#33AA33", "#159925"])
    ...
    stalk(scene, 0.5*scene.width, 0.4*scene.height, 0.97*0.02*scene.height)
    ...
    textile(blueCloth, 0.3*scene.width, 0.1*scene.height, 0.6*scene.width, 0.5*scene.height, 1, 8, ["#3333BB", "#1533BB"])
    let e = blueCloth.get()
    e.mask(flowerMask)    
    ...
    let [sew, wat] = flower(scene, 0.5*scene.width, 0.3*scene.height, 0.97*0.15*scene.height, true)
