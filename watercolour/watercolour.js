import { Command, GUI, Float, Key, Control } from "../libraries/gui/gui.js";
...
const sketch = (s) => {
  ...
  function drawDots() {
    for (let d = 0; d < dots.length; d++) {
      let dot = dots[d];
      s.push();
      let { colour1, colour2, poly, pos, scale } = dot;
      let [x, y] = pos;
      s.translate(x, y);
      for (let i = 0; i < 7; i++) {
        poly = subdivide(poly, 10);
      }
      let lerped = s.lerpColor(colour1, colour2, s.random(cnt / 200.0));
      s.fill(lerped);
      drawPolygon(poly);
      s.pop();
    }
  ...
  function polygon(x, y, radius, npoints) {
    ...
    for (let a = 0; a < s.TWO_PI; a += angle) {
      ...
      let sy =
        s.random(y / 2, y) + s.sin(s.random(a)) * s.random(radius / 2, radius);
      pts.push([sx, sy]);
    }
    return pts;
  }
  ...
  function createGUI() {
    ...
    let alphaMaxControl = new Control(
      [decAM, incAM],
      "+/- max alpha",
      alphaMaxFloat,
    );
    ...
    let alphaMinControl = new Control(
      [decAm, incAm],
      "+/- min alpha",
      alphaMinFloat,
    );
    ...
    let gui = new GUI(
      "Watercolour, RB 2020/05",
      info,
      subinfo,
      [resetCanvasCmd, saveCmd],
