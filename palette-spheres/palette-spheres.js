...
const sketch = (s) => {
    ...
    const br = scene.brightness(color)
      ...
      for(let i=0;i<C_STEPS;i++){
        ...
        for(let j=0;j<SHADOW_STEPS;j++){
          ...
          const col = scene.color(0, 0, 0, alfactor*smoothStep(0,SHADOW_STEPS , j)) // darkens j -> SHADOW_STEPS
  ...
  let capture, captureIt = false, started=true
  ...
  const createGUI = (gui) => {
    ...
    cfg.info = "I explored circles-with-shadows on a lazy Saturday and then tried to spice it up and animate"
