  ...
  function drawMountain(scene, params) {
    ...
    for (let i = 0; i < params.ridge.length; i++) {
      ...
      if (i % 2 == 1) {
        ...
        let slope = -0.6 * s.random(-Math.PI / 4, Math.PI / 4)
        for (let j = 0; j < 3; j++) {
          const maxHeight = params.bottom - cy
          slope = -slope
          let r = s.random(0.3 * maxHeight, 0.8 * maxHeight)
          if (j == 2) {
            r = maxHeight
          }
          let x = cx + r * Math.sin(slope)
          let y = cy + r * Math.cos(slope) + shift
          scene.strokeWeight(2)
          scene.line(cx, cy + shift, x, y) // Straight down line
          const L = s.random(0.2 / (j + 1), 0.3 / (j + 1)) // Smooth it the further down it is
          let yL = y + L * slopeL[1]
          let dL = 0
          if (yL > params.bottom) {
            dL = params.bottom - yL
          ...
          scene.line(x, y, x + L * slopeL[0] + dL, yL)
          scene.line(x, y, x + L * slopeR[0], y + L * slopeR[1])
          shift = shift / (j + 1)
          cx = x
          cy = y
        }
    ...
    scene.endShape()
    scene.fill(white)
    //scene.rect(0, params.bottom, scene.width, scene.height)
  }
