...
function straightThread(scene, x, y, width, height, color, length, dly) {
  ...
  scene.beginShape()
  while (en[1] - height < y + length) { // Need an extra one
    const [pt1, pt2] = halfBraid(scene, st, en, 0.8 * width, color, shade, shine, dly) // Right side
    const [pt3, pt4] = halfBraid(scene, st, en, -0.8 * width, color, shade, shine, dly) // Left side
    st[1] = pt4[1]
    en[1] = pt4[1] + height
  }
  scene.endShape()
