...
class FluentGUI {
  ...
  _title() {
    ...
    this.titleSpan = $.cel("span");
    this.titleSpan.classList.add("title");
    this.titleSpan.onclick = () => this.toggle();
    this.titleSpan.append(this.triangle);
    this.titleSpan.innerHTML += this.title;
    this.titleSpan.append(this.spinner);
    this.dom.append(this.titleSpan, $.cel("hr"));
    return false;
  }
  ...
  configureDrag() {
    ...
    const setTranslate = (xPos, yPos, elt) => {
      elt.style.transform = "translate3d(" + xPos + "px, " + yPos + "px, 0)";
    };
  ...
  update(callback) {
    ...
    if (this.cmds) {
      ...
      for (let cmd of this.cmds) this.dom.append(cmd.format());
    }
    let table;
    if (this.states) {
  ...
  dispatch(key) {
    ...
    if (this.states) {
      for (let state of this.states) {
        if (state.keys) {
          let uppers = state.keys.filter(
            (k) => k.originalKey == k.originalKey.toUpperCase(),
          );
          let lowers = state.keys.filter(
            (k) => k.originalKey != k.originalKey.toUpperCase(),
          );
