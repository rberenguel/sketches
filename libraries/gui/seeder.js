...
import { String } from "./variable.js";
...
class Seeder {
  constructor(gui) {
    ...
    let enterSeedCommand = new Command(Z, "manually enter seed (hex)");
    let seedShow = new String(() => this.hex());
    let X = new Key("x", () => {
      this.seed = (window.performance.now() << 0) % 1000000;
      this.gui.mark();
    });
    let seedControl = new Control([X], "new random seed, current", seedShow);
