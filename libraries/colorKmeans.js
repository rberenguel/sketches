...
function colorKmeans(colors, numClusters, error = 50, seed) {
  ...
  for (let i = 0; i < numClusters; i++) {
    ...
    if (seed === undefined) centroids.push(colors[index]);
    else {
      if (i < seed.length) {
        centroids.push(seed[i]);
      } else centroids.push(colors[index]);
    }
  }
  ...
  for (let k = 0; k < centroids.length; k++) {
    assignments[k] = new Array(colors.length).fill([]); // Trade speed for memory
  }
  ...
  for (let k = 0; k < 100; k++) {
    ...
    for (let c = 0; c < assignments.length; c++) {
      if (assignmentsI[c] > 0) {
        newCentroids[newCentroidsI] = averageColor(
          assignments[c],
          assignmentsI[c],
        );
        newCentroidsI++;
