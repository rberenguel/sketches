...
import { getLargeCanvas, signature, mod } from "../libraries/misc.js";
...
const sketch = (s) => {
  ...
  s.preload = () => {
    ...
    const images = [
      {
        src: "palette-spheres-s.png",
        color: "#55555530",
        blendMode: "multiply",
        name: "Palette spheres",
      },
      {
  ...
  s.setup = () => {
    ...
    let idx = cfg.samples.findIndex((img) => img.src.startsWith(imgReq));
    imageIndex = idx >= 0 ? idx : imageIndex;
    idx = cfg.backgrounds.findIndex((img) => img.src.endsWith(bgReq + ".jpg"));
  ...
  s.draw = () => {
    ...
    if (debug) {
      ...
      p.draw([bg.ul, bg.ur, bg.br, bg.bl]);
  ...
  function createGUI() {
    ...
    let subinfo =
      "Using a tweaked version of <a style='color: inherit;' href='https://github.com/wanadev/perspective.js'>perspective.js</a><br/>Doesn't have all my pieces (yet)";
    let S = new Key("s", () => {
    ...
    let backControl = new Control([decB, incB], "Cycle backgrounds", backInfo);
    ...
    let imgControl = new Control([decI, incI], "Cycle pieces", imgInfo);

    let attrInfo = new String(() => cfg.backgrounds[backgroundIndex].attr);
    let attrControl = new Control([], "Background", attrInfo);
