...
const sketch = (s) => {
  ...
  function cycleParticleDistributions() {
    particleDistributionsIndex =
      (particleDistributionsIndex + 1) % particleDistributions.length;
  ...
  class Particle {
    ...
    move(scene) {
      let theta =
        scene.noise(
          ((WFACTOR * this.x) / W) * squiggly + this.rand,
          ((HFACTOR * this.y) / H) * squiggly + this.rand,
        ) *
        PI *
        2;
  ...
  function createParticles(scene, whatever) {
    ...
    for (let x = base; x < scene.width; x += freq) {
      ...
      let c =
        (scene.noise((WFACTOR * x * squiggly) / W) * palette.colors.length) <<
        0;
    ...
    for (let y = base; y < scene.height; y += freq) {
      let stroke = lineStroke;
      let c =
        (scene.noise((HFACTOR * y * squiggly) / H) * palette.colors.length) <<
        0;
      let color = scene.color(palette.colors[c]);
    ...
    for (let i = 0; i < 5; i++) {
      ...
      for (let j = 0; j < freq; j++) {
        for (let th = base; th < 2 * PI; th += (2 * PI) / (800 / freq)) {
          ...
          let stroke = lineStroke;
          let c =
            (scene.noise(x * squiggly, y * squiggly) * palette.colors.length) <<
            0;
          let color = scene.color(palette.colors[c]);
  ...
  function createGUI() {
    ...
    let enterSeedCommand = new Command(Z, "manually enter seed");
    ...
    let gui = new GUI(
      ...
      info,
      subinfo,
      [saveCmd, resetCanvas, enterSeedCommand],
      [
        modeControl,
        seedControl,
        paletteControl,
