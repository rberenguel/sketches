...
import { Command, GUI, String, Key, Control } from "../libraries/gui/gui.js";
...
const sketch = (s) => {
      ...
      for (let j = 0; j <= rows; j++) {
        ...
        if (cnt < MAXITER * 0.5) {
          ...
          lerped = s.lerpColor(
            palette[2],
            palette[3],
            ((cnt - 0.5 * MAXITER) * 1.0) / (0.5 * MAXITER),
          );
        }
        s.stroke(lerped);
        for (var i = 0; i < pts[j].length; i++) {
          ...
          let range = wavy * 0.01;
          let offsetX = s.map(s.random([-1, 1]), -1, 1, -range, range);
          let offsetY = s.map(s.random([-1, 1]), -1, 1, -range, range);
          pt[0] += offsetX;
          pt[1] += offsetY;
        }
        s.endShape();
  ...
  function createGUI() {
    let title = `Going Crazy, RB 2020/05`;
    let info = undefined;
    let subinfo = undefined;
    let S = new Key("s", () => s.saveCanvas("img", "png"));
