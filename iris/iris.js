...
const sketch = (s) => {
	...
	function iris(centerX, centerY, irisRadius, irisBlur, pupilRadius, irisColors){
		...
		darkenHalo(centerX, centerY, irisRadius+irisBlur, irisBlur*0.2, s.color(0))		
		let params = [{center: 0.5, span:1}, {center: 0.3, span:1}, {center: 0.8, span:1}, {center: 0.3, span:0.5}, {center: 0.5, span:0.5}]
		for(let i=0;i<25; i++){
	...
	function shineArc(x, y, r1, r2, alpha, total, reverse=1, size=1, startAngle=s.PI/6){
		...
		for(let i=0;i<total;i++){
			...
			s.stroke(colour)
			s.line(reverse*s1*Math.cos(angle), -reverse*s1*Math.sin(angle), reverse*s2*Math.cos(angle), -reverse*s2*Math.sin(angle))
			s.endShape()
		}
		s.pop()
	}
	...
	function irisPerturbation(x, y, r1, r2, c1, c2, alpha, start, sizeFactor, steps){
		...
		for(let i=0;i<steps;i++){
			...
			s.line(s1*Math.cos(angle), s1*Math.sin(angle), s2*Math.cos(angle), s2*Math.sin(angle))
			s.endShape()
		}
		s.pop()
	}
