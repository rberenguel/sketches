...
const sketch = (s) => {
  ...
  function pseudoblue() {
    ...
    const xmix = (x, y) =>
      ((x * 212281 + y * 384817) & 0x5555555) * 0.003257328990228013;
    const ymix = (x, y) =>
      ((x * 484829 + y * 112279) & 0x5555555) * 0.002004008016032064;
    const s = 6;
    let a, b;
    return (x, y) => {
      let v = 0;
  ...
  const createGUI = (gui) => {
    ...
    cfg.subinfo =
      "Not fully resolution independent because I use a blue noise function on almost every pixel, and that depends on the resolution. Added a texture overlay because otherwise it felt a bit too flat when zoomed";
    cfg.s = s;
    R = new Key("r", () => {
      gui.spin(() => {
        cfg.s.clear();
