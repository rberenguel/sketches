...
const sketch = (s) => {
  ...
  s.preload = () => {
    ...
    cfg.frag = s.loadStrings("sand.frag");
    cfg.vert = s.loadStrings("sand.vert");
  ...
  function create() {
    ...
    palettes.bw = {
      alive: c2v(cfg.s2, cfg.s2.color("black")),
      dead: c2v(cfg.s2, cfg.s2.color("white")),
    };
  ...
  function initializeShaders() {
    ...
    cfg.sh1.setUniform("u_sand_col", color("alive"));
    cfg.sh1.setUniform("u_empty_col", color("dead"));
    cfg.sh2.setUniform("u_sand_col", color("alive"));
    cfg.sh2.setUniform("u_empty_col", color("dead"));
    ...
    cfg.canvas.mousePressed(() => {
      const mx = s.map(s.mouseX, 0, W, 0, W / 2);
    ...
    cfg.shaa.setUniform("u_use_smoothing", false);
  ...
  s.draw = () => {
    let c;
    if (cfg.drawOn) {
      console.log(cfg.drawOn);
      cfg.s1.stroke("black");
      cfg.s1.fill("black");
      cfg.s1.point(cfg.drawOn.x, cfg.drawOn.y);
      cfg.s2.stroke("black");
      cfg.s2.fill("black");
      cfg.s2.point(cfg.drawOn.x, cfg.drawOn.y);
      cfg.drawOn = null;
    }
